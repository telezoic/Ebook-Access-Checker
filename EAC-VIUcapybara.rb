...
# Written by Kristina Spurgin
  ...
  #modified for Ruby, straight up!

# Usage:
# ruby EAC-VIUcapybara.rb [inputfilelocation] [outputfilelocation]
...
#   access checker result
  ...
  #here we switch out 'celerity' for 'capybara/poltergeist'
  
require 'capybara/poltergeist'
require 'CSV'
require 'highline/import'
require 'open-uri'
Capybara.javascript_driver = :poltergeist #and assign . . .
  ...
  puts "  curio  : CBC Curio Collection" 
...
CSV.open(output, "a") do |c|
  ...
  #edit browser timeout for slow loads . . . 
...
b = Capybara::Session.new(:poltergeist) ##calling Capybara/Poltergeist and PhantomJS [under it all] 
...
csv_data.each do |r|
  ...
  if package == "apb"
    ...
    sleeptime = 5 
    if page.include?("Page not found" || "Cette page n'a pas été trouvée")
        access = "Nope!"
    elsif page.include?("We’re sorry! Access to platform content is limited" || "Désolé, l’accès au contenu du portail")
      access = "Nope!"
